...
public class TheService {
  ...
  public boolean deleteConfigMap(String configMapName) {
    try (KubernetesClient client = kubernetesClientFactory.retrieveKubernetesClient()) {
      ...
      return client.configMaps().inNamespace(namespace).delete(configMap);
    }
  }
  ...
  public void deleteCluster() {
    LOGGER.info("About to delete cluster");

    try (KubernetesClient client = kubernetesClientFactory.retrieveKubernetesClient()) {
      client.services().inNamespace(namespace).withName("terracotta").withGracePeriod(0L).delete();
      client.apps().statefulSets().inNamespace(namespace).withName("terracotta").withGracePeriod(0L).delete();
    }
...
//    deleteCRDInstance();
    ...
    LOGGER.info("Successfully deleted all cluster resources");

  }

  public void createCluster(TerracottaClusterConfiguration terracottaClusterConfiguration) {
    LOGGER.info("Create cluster with configuration : " + terracottaClusterConfiguration);
    ...
    try (KubernetesClient client = kubernetesClientFactory.retrieveKubernetesClient()) {
      ...
      io.fabric8.kubernetes.api.model.Service resultService = client.services().inNamespace(namespace).create(terracottaService);
      ...
      StatefulSet statefulSet = new StatefulSetBuilder()
        ...
        .withArgs(Arrays.asList("-f", "/configs/terracotta.xml", "-n", "$(POD_NAME)"))
//        .withImagePullPolicy("Never")
      ...
      StatefulSet resultStatefulSet = client.apps().statefulSets().inNamespace(namespace).create(statefulSet);
  ...
  public String constructTerracottaServerUrl(TerracottaClusterConfiguration terracottaClusterConfiguration) {
    ...
    IntStream.range(0, terracottaClusterConfiguration.getServersPerStripe()).forEach(serverNumber -> {
      stringBuilder.append("terracotta-" + serverNumber + ".terracotta:9410");
